...
$(document).ready(function () {
    ...
    $('#runButton').click(function () {
        if (!$(this).hasClass('inactive')) {
            console.log("in runButton")
            $('#simRunning_progressBar').progressbar("value", 50);
...
function load_data() {
    ...
    $.ajax({
        ...
        setTimeout(function () {
            $('#click_instruction').removeClass('hidden')
        }, 1500);
        setTimeout(function () {
            $('#click_instruction').addClass('hidden')
        }, 6000);
        // Reset the correct title
        document.title = "DONE - ASTRA High Altitude Balloon Flight Planner";
        setTimeout(function () {
            document.title = "ASTRA High Altitude Balloon Flight Planner"
        }, 5000);
    });
